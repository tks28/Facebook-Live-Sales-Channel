<?php
    ...
    foreach($itemName as $item){
        $quantity = 0;
        $sql = "SELECT item, quantity FROM orders WHERE MONTH(date) = $month";
        $result = $conn->query($sql);
        ...
        array_push($itemQuantity, $quantity);
    }

    $counter = 0;
    while($counter < count($itemName)){
        array_push($dataPoints, array("label"=> $itemName[$counter], "y"=>$itemQuantity[$counter]));
        $counter++;
    }
?>
    ...
    <script>
        window.onload = function () {
            var chart = new CanvasJS.Chart("chartContainer", {
                ...
                theme: "light2",
                title:{
                    text: "Monthly Item Quantity Sales Chart"
                },
                axisY:{
                    includeZero: true
                },
